    ...
    def analytics_page():
        ...
        if data:
            ...
            if data_lucro_produto_dia:
                ...
                product_profit = df_lucro_produto_dia.groupby('Produto')['Total_Lucro'].sum().sort_values(ascending=False)
                ordered_products = product_profit.index.tolist()

                # Definir a ordem dos produtos como categórico ordenado
                df_lucro_produto_dia['Produto'] = pd.Categorical(
                    df_lucro_produto_dia['Produto'],
