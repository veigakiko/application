...
def run_query(query: str, values=None, commit: bool = False):
    ...
    try:
        ...
        st.error(f"Erro ao executar a query: {e.pgerror}")
    finally:
        if not conn.closed:
            conn.close()
    return None
...
def load_all_data():
    ...
    try:
        ...
        data["clients"] = run_query(
            'SELECT DISTINCT "Nome_Completo" FROM public.tb_clientes ORDER BY "Nome_Completo"'
        ) or []
                    ...
                    if len(match) > 1:
                        ...
                        if delete_btn:
                            q_del = """
                                ...
                                WHERE "Cliente"=%s AND "Produto"=%s AND "Data"=%s AND "Status"=%s
                            """
                            run_query(q_del, (original_client, original_product, original_date, original_status), commit=True)
                        ...
                        if update_btn:
                            ...
                            run_query(q_upd, (
                                ...
                                original_client, original_product, original_date, original_status
                            ), commit=True)
        ...
        if submit_prod:
            if supplier and product and quantity > 0 and unit_value >= 0:
                ...
                q_ins = """
                    ...
                    ("Supplier", "Product", "Quantity", "Unit_Value", "Total_Value", "Creation_Date", "Image_URL")
...
def events_calendar_page():
    ...
    # Destacar dias com eventos usando BeautifulSoup para maior robustez
